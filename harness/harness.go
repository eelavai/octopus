...
var people = []string{"siddarth", "gdb", "christian", "andy"}
...
func (h *Harness) issueQuery(node *agent.Agent, query string) {
	...
	start := time.Now()
	resp, err := h.client.SafePost(node.ConnectionString, "/sql", b)
	end := time.Now()

	if err != nil {
		switch err := err.(type) {
		case *transport.RequestError:
			log.Printf("[harness] HTTP %d status code from %s (in response to %#v): %s. Thread sleeping 100ms...", err.StatusCode, node, query, err.Message)
		default:
			if h.nodeBooted(node) {
				log.Printf("[harness] Request error from %s (in response to %#v): %s. Thread sleeping 100ms...", node, query, err)
			}
		}

		// Guess we should back off for a sec (well, 100ms to
		// be precise).
		time.Sleep(100 * time.Millisecond)
		return
	}
